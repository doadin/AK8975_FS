      ...
      mHasPendingEvent(false)
{
    ...
    if (data_fd >= 0) {
		...
		LOGD("AdxlSensor: sysfs_path=%s", input_sysfs_path);
    } else {
...
int AdxlSensor::setEnable(int32_t handle, int enabled) {
	...
	/* handle check */
	if (handle != ID_A) {
		LOGE("AdxlSensor: Invalid handle (%d)", handle);
		return -EINVAL;
	}
    ...
    if (buffer[0] != '\0') {
		...
		LOGD("AdxlSensor: Control set %s", buffer);
   		setInitialState();
    }
	...
	LOGD("AdxlSensor: mEnabled = %d", mEnabled);
...
int AdxlSensor::setDelay(int32_t handle, int64_t delay_ns)
{
	...
	if (mDelay != delay_ns) {
		...
		if (err == 0) {
			mDelay = delay_ns;
			LOGD("AdxlSensor: Control set delay %f ms requetsed, using %f ms",
...
int AdxlSensor::readEvents(sensors_event_t* data, int count)
{
    ...
    while (count && mInputReader.readEvent(&event)) {
        ...
        if (type == EV_ABS) {
            ...
            LOGE("AdxlSensor: unknown event (type=%d, code=%d)",
                    type, event->code);
        }
        mInputReader.next();
    }
